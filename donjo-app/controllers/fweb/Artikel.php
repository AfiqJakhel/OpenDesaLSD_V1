...
class Artikel extends Web_Controller
{
    ...
    */
    ...
    public function berita(): void
    {
        ...
        $kategori_id     = trim(request()->get('kategori'));
        $data['title']   = 'Berita & Artikel';
        $data['cari']    = $cari;

        $artikel = ModelsArtikel::withOnly(['author', 'category'])
            ->enable()
            ->sitemap()
            ->artikelStatis()
            ->when($cari, static fn ($q) => $q->cari($cari))
            ->when($kategori_id, static fn ($q) => $q->kategori($kategori_id))
            ->orderBy('tgl_upload', 'desc')
            ->paginate(10);

        if (! $artikel->isEmpty()) {
            $artikel->getCollection()->transform(static function ($item) {
        ...
        view('theme::partials.berita.index', $data);
    }
