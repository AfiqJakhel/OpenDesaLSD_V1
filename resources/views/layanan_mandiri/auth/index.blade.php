...
<head>
    <meta charset="utf-8">
    <title>{{ setting('login_title') . ' ' . ucwords(setting('sebutan_desa')) . ($desa['nama_desa'] ? ' ' . $desa['nama_desa'] : '') . get_dynamic_title_page_from_path() }}</title>
    ...
    <div class="relative z-10 w-full max-w-4xl">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-0 bg-white rounded-3xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] overflow-hidden">
                    ...
                    <div class="relative z-10">
                        <div class="inline-flex items-center gap-2 mb-8">
                            <div class="w-12 h-12 bg-white/10 backdrop-blur-sm rounded-xl flex items-center justify-center border border-white/20">
                                <img src="{{ gambar_desa($desa['logo']) }}" alt="Logo" class="w-8 h-8 object-contain brightness-0 invert">
                            </div>
                            <div>
                        ...
                        <h1 class="text-white leading-tight mb-4 text-center mt-6">
                            ...
                            <span class="text-accent" style="font-size: 1.5rem; font-weight: 800; display: block; line-height: 1.2;">{{ ucwords(setting('sebutan_desa')) }} {{ $desa['nama_desa'] }}</span>
                        </h1>
            ...
            <div>
                <div class="login-card p-10 md:p-12 h-full flex flex-col justify-center">
                    ...
                    <div class="md:hidden text-center mb-6">
                        <div class="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-primary to-primary/90 rounded-2xl shadow-lg mb-4">
                            <img src="{{ gambar_desa($desa['logo']) }}" alt="Logo" class="w-10 h-10 object-contain brightness-0 invert">
                        </div>
                        <h2 class="text-2xl font-bold text-gray-900">Layanan Mandiri</h2>
                        <p class="text-sm text-gray-600 mt-1">{{ ucwords(setting('sebutan_desa')) }} {{ $desa['nama_desa'] }}</p>
                    </div>
                ...
                @if ($errors->any())
                    <div @if (!str_contains($errors->first('email'), 'Terlalu banyak upaya masuk.')) id="notif" @endif class="mb-6 bg-red-50 border-l-4 border-red-500 text-red-800 px-4 py-3 rounded-lg">
                ...
                <div class="mt-8 pt-6 border-t border-gray-100 text-center space-y-3">
                    ...
                    <a href="" target="_blank" class="inline-flex items-center gap-2 text-xs text-gray-400 hover:text-primary transition">
                        ...
                        <span>OpenSID v{{ AmbilVersi() }}</span>
                    </a>
                    <!-- Footer Nagari -->
