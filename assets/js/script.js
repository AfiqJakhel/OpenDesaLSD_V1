...
$(document).ready(function() {
    ...
    $("ul.sidebar-menu").on("expanded.tree", function(e) {
        ...
        // setTimeout(scrollTampil($("li.treeview.menu-open")[0]), 500); // Dinonaktifkan agar layar tidak otomatis menggulir atau fokus
    });
...
// Agar dropdown di dalam grid/tabel yang responsif (memiliki overflow) dapat terlihat seutuhnya di luar area tabel
// dan otomatis mendorong pagination (atau elemen bawah lainnya) bergeser ke bawah jika melebihi batas
$(document).on("show.bs.dropdown", ".table-responsive", function(e) {
    var $dropdownMenu = $(e.target).find(".dropdown-menu");
    var $buttonGroup = $(e.target);
    var $tableResponsive = $(this);
    
    // Simpan referensi parent aslinya untuk dikembalikan saat ditutup
    $dropdownMenu.data('original-parent', $buttonGroup);
    
    // Kalkulasi cepat tinggi dropdown (sementara clone untuk mengukur hidden dropdown)
    var $clone = $dropdownMenu.clone().css({display: 'block', visibility: 'hidden', position: 'absolute'}).appendTo('body');
    var menuHeight = $clone.outerHeight() || 150;
    $clone.remove();
    
    // Hitung apakah posisi paling bawah dropdown nantinya menabrak batas bawah tabel
    var $table = $buttonGroup.closest('table');
    var targetBottom = $table.length ? ($table.offset().top + $table.outerHeight()) : ($tableResponsive.offset().top + $tableResponsive.outerHeight());

    var eOffset = $buttonGroup.offset();
    var buttonHeight = $buttonGroup.outerHeight();
    var dropdownBottom = eOffset.top + buttonHeight + menuHeight;
    
    // Jika dropdown lebih ke bawah daripada batas tabel, dorong kontainer di bawahnya (seperti pagination) ke bawah
    if (dropdownBottom > targetBottom) {
        var overflowAmount = dropdownBottom - targetBottom;
        var $targetContainer = $table.length ? $table.parent() : $tableResponsive;
        $targetContainer.css("padding-bottom", overflowAmount + 15 + "px"); // tambah jarak aman 15px
    }
    
    // Detach dan append dropdown ke luar container overflow, ke <body>
    $("body").append($dropdownMenu.detach());
    
    // Posisikan secara absolut dengan kordinat menu
    $dropdownMenu.css({
        'display': 'block',
        'top': eOffset.top + buttonHeight,
        'left': eOffset.left,
        'position': 'absolute',
        'z-index': 9999
    });
...
$(document).on("hide.bs.dropdown", ".table-responsive", function(e) {
    ...
    // Kembalikan ukuran batas bawah tabel ke normal
    var $table = $buttonGroup.closest('table');
    var $targetContainer = $table.length ? $table.parent() : $tableResponsive;
    $targetContainer.css("padding-bottom", "");
    $tableResponsive.css("margin-bottom", ""); // Fallback untuk memastikan bersih
    
    // Temukan kembali dropdown spesifik yang didetach dari body
    var $dropdownMenu = $("body").children(".dropdown-menu").filter(function() {
        var parent = $(this).data('original-parent');
        return parent && parent[0] === $buttonGroup[0];
    });
    
    if (!$dropdownMenu.length) {
        $dropdownMenu = $("body").children(".dropdown-menu").last(); // fallback
    }
    
    // Pasang kembali ke grup parent asalnya
    if ($dropdownMenu.length) {
        $dropdownMenu.css({
            'display': '',
            'top': '',
        ...
        $buttonGroup.append($dropdownMenu.detach());
    }
});
...
// =========================================================
// MODERN DASHBOARD: INTERACTIONS & ANIMATIONS
// =========================================================
$(document).ready(function() {
    // 1. Dinamis Navbar Shadow on Scroll
    var header = $('.main-header .navbar');
    if (header.length > 0) {
        $(window).scroll(function() {
            if ($(window).scrollTop() > 15) {
                // Shadow lebih tegas ketika discroll
                header.css("box-shadow", "0 10px 15px -3px rgba(0, 0, 0, 0.1)");
            } else {
                // Kembali ke soft shadow
                header.css("box-shadow", "var(--shadow-sm)");
            }
        });
    }

    // 2. Efek Ripple Modern pada Tombol
    $(document).on('click', '.btn', function(e) {
        var $btn = $(this);
        // Only apply ripple to primary buttons or normal btn, exclude btn-xs or icons to avoid overflow bugs if not positioned relative
