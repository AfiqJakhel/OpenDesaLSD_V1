...
<style>
    /* CSS dihapus karena sudah menggunakan Tailwind */
</style>
<?php $__env->stopPush(); ?>

<main class="w-full bg-[#F7F8FA] w-full flex flex-col items-center justify-center relative overflow-hidden" style="min-height: calc(100vh - 200px);">
    ...
    <div class="relative flex flex-col items-center justify-center gap-5 z-10 w-full px-4 text-center">
        ...
        <div class="flex flex-col items-center gap-5 mt-[-20px]">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-[12px] mt-2">
                <a href="<?php echo e(site_url()); ?>" class="w-full sm:w-auto bg-[#0D2247] text-white font-bold rounded-lg flex items-center justify-center transition-all duration-200" style="padding: 14px 32px;" onmouseover="this.style.backgroundColor='#1a3a6e'; this.style.transform='translateY(-2px)'; this.style.boxShadow='0 8px 24px rgba(13,34,71,0.25)'" onmouseout="this.style.backgroundColor='#0D2247'; this.style.transform='none'; this.style.boxShadow='none'">
                    Kembali ke Beranda
                </a>
                <a href="mailto:<?php echo e($desa['email_desa'] ?? 'kontak@desa.id'); ?>" class="w-full sm:w-auto border-[1.5px] border-[#0D2247] bg-transparent text-[#0D2247] hover:bg-[#0D2247] hover:text-white font-bold rounded-lg flex items-center justify-center transition duration-200" style="padding: 14px 32px;">
                    Hubungi Kami
                </a>
            </div>
        </div>
    </div>
</main><?php /**PATH C:\xampp\htdocs\OpenSID\/storage/app/themes/esensi/resources/views/commons/404.blade.php ENDPATH**/ ?>