<section class="sliderx w-full relative group transition-all duration-300 overflow-hidden" x-data="heroCarousel()">
        ...
        <?php $__currentLoopData = $slider_gambar['gambar']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $data): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <?php if(is_file($img)): ?>
                <figure class="absolute inset-0 h-48 lg:h-[400px] w-full transition-all duration-600 ease-in-out"
                        :class="currentSlide === <?php echo e($slideIndex); ?> ? 'opacity-100 translate-x-0' : (currentSlide > <?php echo e($slideIndex); ?> ? 'opacity-0 -translate-x-full' : 'opacity-0 translate-x-full')"
                    ...
                    <?php if($slider_gambar['sumber'] != 3): ?>
                        <div class="absolute bg-black bg-opacity-60 bottom-0 left-0 right-0 text-white">
                            <a href="<?php echo e(site_url('artikel/' . buat_slug($data))); ?>" class="font-bold text-h5 block py-4 px-5 hover:py-5 transition-all duration-300"><?php echo e($data['judul']); ?></a>
                        </div>
                    <?php endif; ?>
                </figure>
                <?php $slideIndex++; ?>
    ...
    <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-[99] flex gap-2">
        <?php for($i = 0; $i < $slideIndex; $i++): ?>
            <button @click="goToSlide(<?php echo e($i); ?>)" 
                    class="w-2 h-2 rounded-full transition-all duration-300 cursor-pointer"
                    :class="currentSlide === <?php echo e($i); ?> ? 'bg-white w-8' : 'bg-white/40 hover:bg-white/60'">
            </button>
        <?php endfor; ?>
    </div>
</section>
...
<?php $__env->stopPush(); ?>
<?php /**PATH C:\xampp\htdocs\OpenSID\/storage/app/themes/esensi/resources/views/partials/slider.blade.php ENDPATH**/ ?>