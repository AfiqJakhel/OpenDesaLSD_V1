...
<div id="tab-rpjm">
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            {{-- Header Bidang --}}
            <div class="flex items-center gap-4 px-6 py-5 cursor-pointer" onclick="toggleBidang({{ $i }})"
                 style="border-left:4px solid {{ $b['color'] }};">
                <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0"
                     style="background:{{ $b['bg'] }};">
                    <i class="fas {{ $b['icon'] }}" style="color:{{ $b['color'] }};"></i>
                </div>
                <div class="flex-1">
                    <div class="font-bold text-[#0D2247] text-sm">Bidang {{ $i+1 }}: {{ $b['nama'] }}</div>
                    <div class="flex items-center gap-3 mt-2">
                        <div class="progress-bar flex-1 max-w-xs">
                            <div class="progress-fill" style="width:{{ $b['persen'] }}%;background:{{ $b['color'] }};"></div>
                        </div>
                        <span class="text-xs font-bold" style="color:{{ $b['color'] }};">{{ $b['persen'] }}% alokasi</span>
                    </div>
                </div>
                <div class="text-xs text-gray-400">{{ count($b['program']) }} program</div>
                <i class="fas fa-chevron-down text-gray-400 transition-transform" id="chevron-{{ $i }}"></i>
            </div>

            {{-- Program List --}}
            <div id="bidang-{{ $i }}" class="hidden border-t border-gray-100">
                <div class="divide-y divide-gray-50">
                    @foreach($b['program'] as $j => $prog)
                    <div class="flex items-start gap-4 px-6 py-4 hover:bg-gray-50 transition">
                        <div class="w-7 h-7 rounded-lg flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-0.5"
                             style="background:{{ $b['color'] }};">{{ $j+1 }}</div>
                        <div class="flex-1">
                            <p class="text-gray-800 text-sm font-semibold">{{ $prog['nama'] }}</p>
                            <div class="flex items-center gap-3 mt-1.5">
                                <span class="text-[10px] text-gray-400"><i class="fas fa-map-pin mr-1"></i>{{ $prog['sasaran'] }}</span>
                                <span class="text-[10px] text-gray-400"><i class="fas fa-clock mr-1"></i>{{ $prog['tahun'] }}</span>
                            </div>
                        </div>
                        <span class="bid-badge flex-shrink-0" style="color:{{ $b['color'] }};background:{{ $b['bg'] }};">
                            {{ explode(' ', trim($b['nama']))[0] }}
                        </span>
                    </div>
                    @endforeach
                </div>
...
{{-- ══ TAB 2: REALISASI FISIK (dari API OpenSID) ══ --}}
