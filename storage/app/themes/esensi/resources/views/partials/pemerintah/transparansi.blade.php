...
<section class="relative bg-[#0D2247] text-white overflow-hidden" style="min-height: 260px;">
    ...
    <div class="container mx-auto px-4 max-w-7xl py-14 relative z-10">
        ...
        <span class="text-amber-400 font-bold tracking-[0.2em] text-xs uppercase font-jakarta">Keuangan Nagari</span>
...
<div class="bg-white shadow-sm sticky top-[96px] z-40 border-b border-gray-100">
    <div class="container mx-auto px-4 max-w-7xl overflow-x-auto scrollbar-none">
        <div class="flex">
            ...
            <a href="{{ site_url('pemerintah/regulasi') }}" class="gov-tab"><i class="fas fa-gavel text-[11px]"></i> Regulasi</a>
            <a href="{{ site_url('pemerintah/transparansi') }}" class="gov-tab active"><i class="fas fa-chart-pie text-[11px]"></i> Transparansi</a>
        </div>
    </div>
</div>
...
<div class="container mx-auto px-4 max-w-6xl">
    ...
    <div class="flex items-center justify-between mb-10 flex-wrap gap-4">
        <div>
            <h2 class="font-jakarta font-extrabold text-3xl text-[#0D2247]">APBNag {{ $apb['tahun'] }}</h2>
            <p class="text-gray-500 text-sm mt-1">Diperbarui: {{ date('d F Y') }}</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <a href="{{ site_url('apbdesa') }}" class="bg-[#0D2247] text-white text-xs px-4 py-2 rounded-xl font-jakarta font-bold hover:bg-[#162d5a] transition">
                APBDesa Lengkap →
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-12">
        <div class="bg-white rounded-3xl p-6 border border-gray-100 shadow-md text-center">
            ...
            <div class="font-jakarta font-extrabold text-[#0D2247] text-xl leading-tight">Rp {{ number_format($apb['total_pendapatan'] / 1000000, 1) }} Jt</div>
            <div class="text-gray-400 text-xs mt-1">Tahun {{ $apb['tahun'] }}</div>
        </div>
        <div class="bg-white rounded-3xl p-6 border border-gray-100 shadow-md text-center">
            <div class="text-[10px] font-jakarta font-bold uppercase tracking-widest text-gray-400 mb-2">Realisasi Pendapatan</div>
            <div class="font-jakarta font-extrabold text-[#3a7d44] text-xl leading-tight">{{ $pct_realisasi_pendapatan }}%</div>
            <div class="text-gray-400 text-xs mt-1">Rp {{ number_format($apb['realisasi_pendapatan'] / 1000000, 1) }} Jt</div>
        </div>
        <div class="bg-white rounded-3xl p-6 border border-gray-100 shadow-md text-center">
            <div class="text-[10px] font-jakarta font-bold uppercase tracking-widest text-gray-400 mb-2">Realisasi Belanja</div>
            <div class="font-jakarta font-extrabold text-[#c8973a] text-xl leading-tight">{{ $pct_realisasi_belanja }}%</div>
            <div class="text-gray-400 text-xs mt-1">Rp {{ number_format($apb['realisasi_belanja'] / 1000000, 1) }} Jt</div>
        </div>
        <div class="bg-white rounded-3xl p-6 border border-gray-100 shadow-md text-center">
            <div class="text-[10px] font-jakarta font-bold uppercase tracking-widest text-gray-400 mb-2">Sisa Anggaran</div>
            <div class="font-jakarta font-extrabold text-[#8b3a2a] text-xl leading-tight">Rp {{ number_format(($apb['total_belanja'] - $apb['realisasi_belanja']) / 1000000, 1) }} Jt</div>
            <div class="text-gray-400 text-xs mt-1">Belum terealisasi</div>
        </div>
    </div>

    {{-- Dua kolom: Pendapatan & Belanja --}}
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
        ...
        <div class="bg-white rounded-3xl p-8 border border-gray-100 shadow-md">
            <div class="flex items-center justify-between mb-6">
                ...
                <span class="text-sm font-jakarta font-extrabold text-[#3a7d44]">{{ $pct_realisasi_pendapatan }}% terealisasi</span>
            </div>
            ...
            <div class="mb-6 p-4 bg-green-50 rounded-2xl">
                <div class="flex justify-between text-xs font-jakarta mb-2">
                    ...
                    <span class="font-bold text-[#3a7d44]">Rp {{ number_format($apb['realisasi_pendapatan'] / 1000000, 1) }} Jt / {{ number_format($apb['total_pendapatan'] / 1000000, 1) }} Jt</span>
                </div>
                <div class="budget-bar-track">
                    <div class="budget-bar-fill" style="width: {{ $pct_realisasi_pendapatan }}%; background: linear-gradient(90deg, #3a7d44, #5ab46b);"></div>
                </div>
            </div>

            <div class="space-y-5">
                @foreach($apb['pendapatan'] as $item)
                @php
                    $pct = round(($item['realisasi'] / $item['anggaran']) * 100);
                @endphp
                <div>
                    <div class="flex justify-between items-start mb-2 gap-2">
                        <span class="text-xs font-jakarta font-semibold text-gray-700 leading-tight flex-1">{{ $item['label'] }}</span>
                        <span class="text-xs font-jakarta font-extrabold flex-shrink-0" style="color: {{ $item['color'] }};">{{ $pct }}%</span>
                    </div>
                    <div class="budget-bar-track">
                        <div class="budget-bar-fill" style="width: {{ $pct }}%; background: {{ $item['color'] }};"></div>
                    </div>
                    <div class="flex justify-between mt-1 text-[10px] text-gray-400 font-jakarta">
                        <span>Realisasi: Rp {{ number_format($item['realisasi'] / 1000000, 1) }} Jt</span>
                        <span>Target: Rp {{ number_format($item['anggaran'] / 1000000, 1) }} Jt</span>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="bg-white rounded-3xl p-8 border border-gray-100 shadow-md">
            <div class="flex items-center justify-between mb-6">
                ...
                <span class="text-sm font-jakarta font-extrabold text-[#c8973a]">{{ $pct_realisasi_belanja }}% terealisasi</span>
            </div>
            ...
            <div class="mb-6 p-4 bg-amber-50 rounded-2xl">
                <div class="flex justify-between text-xs font-jakarta mb-2">
                    <span class="text-gray-600 font-semibold">Realisasi Total</span>
                    <span class="font-bold text-[#c8973a]">Rp {{ number_format($apb['realisasi_belanja'] / 1000000, 1) }} Jt / {{ number_format($apb['total_belanja'] / 1000000, 1) }} Jt</span>
                </div>
                <div class="budget-bar-track">
                    <div class="budget-bar-fill" style="width: {{ $pct_realisasi_belanja }}%; background: linear-gradient(90deg, #c8973a, #e0b060);"></div>
                </div>
            </div>

            <div class="space-y-5">
                @foreach($apb['belanja'] as $item)
                @php
                    $pct = round(($item['realisasi'] / $item['anggaran']) * 100);
                @endphp
                <div>
                    ...
                    <div class="flex justify-between mt-1 text-[10px] text-gray-400 font-jakarta">
                        <span>Realisasi: Rp {{ number_format($item['realisasi'] / 1000000, 1) }} Jt</span>
                        <span>Pagu: Rp {{ number_format($item['anggaran'] / 1000000, 1) }} Jt</span>
                    </div>
                </div>
                @endforeach
            </div>
    ...
    <div class="bg-[#0D2247] rounded-3xl p-10 text-white">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <div class="flex flex-col gap-3">
                <a href="{{ site_url('apbdesa') }}" class="flex items-center gap-3 bg-white/10 hover:bg-white/20 rounded-2xl p-4 transition group">
                ...
                <a href="{{ site_url('informasi-publik') }}" class="flex items-center gap-3 bg-white/10 hover:bg-white/20 rounded-2xl p-4 transition group">
