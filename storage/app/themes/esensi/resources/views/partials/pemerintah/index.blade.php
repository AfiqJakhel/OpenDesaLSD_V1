...
<div class="bg-white shadow-sm sticky top-[96px] z-40 border-b border-gray-100">
    <div class="container mx-auto px-4 max-w-7xl overflow-x-auto scrollbar-none">
        <div class="flex font-jakarta">
            <a href="{{ site_url('pemerintah') }}"                  class="gov-tab active"><i class="fas fa-users text-[10px]"></i> Aparatur Nagari</a>
            <a href="{{ site_url('sotk') }}"                        class="gov-tab"><i class="fas fa-sitemap text-[10px]"></i> Struktur Organisasi</a>
            <a href="{{ site_url('pemerintah/tugas-fungsi') }}"     class="gov-tab"><i class="fas fa-tasks text-[10px]"></i> Tugas &amp; Fungsi</a>
            <a href="{{ site_url('pemerintah/regulasi') }}"         class="gov-tab"><i class="fas fa-gavel text-[10px]"></i> Regulasi</a>
            <a href="{{ site_url('pemerintah/transparansi') }}"     class="gov-tab"><i class="fas fa-chart-pie text-[10px]"></i> Transparansi</a>
        </div>
    </div>
</div>
...
<div class="container mx-auto px-4 max-w-7xl py-14 font-jakarta space-y-16">
            ...
            <div class="flex flex-col md:flex-row items-center gap-0">
                ...
                <div class="flex-1 p-8 text-white">
                    <div class="inline-flex items-center gap-2 bg-amber-500/20 text-amber-400 px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest mb-4">
                        <i class="fas fa-star text-[9px]"></i> {{ $wali['jabatan'] }}
                    </div>
                    <h3 class="font-extrabold text-2xl text-white mb-1">{{ $wali['nama'] }}</h3>
                    @if($wali['niap'])
                    <p class="text-white/40 text-xs mb-4">NIAP: {{ $wali['niap'] }}</p>
                    @endif
                    ...
                    {{-- Sosmed --}}
                    @if(!empty($wali['sosmed']))
                    <div class="flex gap-2">
                        @foreach($wali['sosmed'] as $key => $val)
                        @if($val)
    ...
    <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            @foreach($staf as $p)
            @php [$fgColor, $bgColor] = badgeColor($p['jabatan']); @endphp
            <div class="card-perangkat group">
                <div class="h-48 overflow-hidden bg-gray-100 relative">
                    ...
                    @else
                        <div class="w-full h-full flex items-center justify-center" style="background:{{ $fgColor }}15;">
                            <i class="fas fa-user text-5xl" style="color:{{ $fgColor }}30;"></i>
                        </div>
                    @endif
                ...
                <div class="p-5 text-center">
                    <span class="badge-jabatan mb-3" style="color:{{ $fgColor }};background:{{ $bgColor }};">
                        {{ $p['jabatan'] }}
                    </span>
                    <h4 class="font-bold text-gray-800 text-sm mt-2 leading-snug">{{ $p['nama'] }}</h4>
                    @if($p['niap'])
                    <p class="text-gray-400 text-[11px] mt-1">NIAP: {{ $p['niap'] }}</p>
                    @endif
                    @if(!empty($p['sosmed']))
                    <div class="flex justify-center gap-2 mt-3">
                        @foreach($p['sosmed'] as $key => $val)
                        @if($val)
                        <a href="{{ $val }}" target="_blank"
                           class="soc-btn bg-gray-50 hover:text-white transition"
                           style="color:{{ $fgColor }};--hover-bg:{{ $fgColor }};"
                           onmouseover="this.style.background='{{ $fgColor }}';this.style.color='white';"
                           onmouseout="this.style.background='';this.style.color='{{ $fgColor }}';">
                            <i class="fab fa-{{ $key }}"></i>
                        </a>
                        @endif
                        @endforeach
            ...
            <div class="card-perangkat group text-center">
                ...
                <div class="p-4">
                    <span class="badge-jabatan text-[9px]" style="color:#7c3d8f;background:#f5edf7;">
                        {{ str_replace('Kepala Jorong ', '', $j['jabatan']) }}
                    </span>
                    <p class="font-bold text-gray-800 text-xs mt-2 leading-snug">{{ $j['nama'] }}</p>
                </div>
            </div>
            @endforeach
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
        ...
        <div class="flex flex-col items-center text-center">
            <div class="w-48 h-48 rounded-3xl overflow-hidden shadow-2xl ring-4 ring-[#0D2247]/10 mb-5">
                @if($wali && $wali['foto'])
                    <img src="{{ $wali['foto'] }}" alt="{{ $wali['nama'] ?? 'Wali Nagari' }}" class="w-full h-full object-cover">
                @else
            ...
            <div class="bg-[#0D2247] text-white rounded-2xl px-6 py-4 w-full">
                ...
                <div class="font-extrabold text-lg">{{ $wali['nama'] ?? 'Syamsul Akmal' }}</div>
        ...
        <div class="lg:col-span-2">
            ...
            <div class="mt-8 pt-6 border-t border-gray-100 flex items-center gap-4">
                ...
                <div>
                    <div class="font-extrabold text-[#0D2247]">{{ $wali['nama'] ?? 'Syamsul Akmal' }}</div>
