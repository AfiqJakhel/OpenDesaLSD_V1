...
<div class="bg-white shadow-sm sticky top-[96px] z-40 border-b border-gray-100">
    <div class="container mx-auto px-4 max-w-7xl overflow-x-auto scrollbar-none">
        <div class="flex">
            <a href="{{ site_url('pemerintah') }}" class="gov-tab">
                <i class="fas fa-users text-[11px]"></i> Aparatur Nagari
            </a>
            <a href="{{ site_url('sotk') }}" class="gov-tab">
                <i class="fas fa-sitemap text-[11px]"></i> Struktur Organisasi
            </a>
            <a href="{{ site_url('pemerintah/tugas-fungsi') }}" class="gov-tab active">
                <i class="fas fa-tasks text-[11px]"></i> Tugas & Fungsi
            </a>
            <a href="{{ site_url('pemerintah/regulasi') }}" class="gov-tab">
                <i class="fas fa-gavel text-[11px]"></i> Regulasi
            </a>
            <a href="{{ site_url('pemerintah/transparansi') }}" class="gov-tab">
...
<div class="container mx-auto px-4 max-w-5xl">
    ...
    <div class="space-y-6">
        @foreach($tugas_jabatan as $idx => $item)
        <div class="tugas-card">
            <div class="grid grid-cols-1 md:grid-cols-4">
                {{-- Header Jabatan --}}
                <div class="p-7 flex flex-col items-center justify-center text-center" style="background: {{ $item['bg'] }}11; border-right: 1.5px solid {{ $item['color'] }}22;">
                    <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-white text-xl mb-4 shadow-lg" style="background: {{ $item['color'] }};">
                        <i class="fas {{ $item['icon'] }}"></i>
                    </div>
                    <h3 class="font-jakarta font-extrabold text-sm text-center leading-tight" style="color: {{ $item['color'] }};">{{ $item['jabatan'] }}</h3>
                    <p class="text-gray-500 text-xs mt-3 leading-relaxed font-jakarta">{{ $item['singkat'] }}</p>
                </div>
                ...
                <div class="md:col-span-3 p-7">
                    ...
                    <ul class="space-y-3">
                        ...
                        <li class="flex items-start gap-3">
                            <div class="w-5 h-5 rounded-lg flex-shrink-0 flex items-center justify-center mt-0.5" style="background: {{ $item['color'] }}18;">
                                <i class="fas fa-check text-[9px]" style="color: {{ $item['color'] }};"></i>
                            </div>
                            <span class="text-gray-700 text-sm leading-relaxed font-jakarta">{{ $t }}</span>
                        </li>
                        @endforeach
                    </ul>
                </div>
