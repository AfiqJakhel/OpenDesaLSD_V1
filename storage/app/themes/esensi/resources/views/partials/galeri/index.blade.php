...
<section class="relative bg-[#0D2247] text-white overflow-hidden font-jakarta" style="min-height:220px;">
    ...
    <div class="container mx-auto px-4 max-w-7xl py-14 relative z-10">
        ...
        <nav class="flex items-center gap-2 text-xs mb-5">
            ...
            @if(isset($parent))
                <a href="{{ site_url('galeri') }}" class="text-white/60 hover:text-white transition">Galeri</a>
                <i class="fas fa-chevron-right text-[8px] text-white/30"></i>
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                @if(isset($parent))
                    ...
                    <h1 class="text-4xl font-extrabold drop-shadow-md">{{ $title }}</h1>
                    @if(isset($photos))
                        <p class="text-white/60 text-sm mt-2">{{ $photos->count() }} foto dalam album ini</p>
                    @endif
                @else
                    ...
                    @if(isset($albums))
                        <p class="text-white/60 text-sm mt-2">{{ $albums->count() }} album tersedia</p>
                    @endif
                @endif
            </div>
        </div>
...
<div class="container mx-auto px-4 max-w-7xl py-10">
    ...
    @if(isset($parent))
        {{-- ── HALAMAN DETAIL: Foto dalam Album ── --}}
        @if(isset($photos) && $photos->count() > 0)
            <div class="masonry-grid">
                @foreach($photos as $foto)
                    <div class="masonry-item">
                        <a class="photo-card block"
                           onclick="openLightbox({{ $loop->index }}, event)"
                           data-index="{{ $loop->index }}"
                           data-src="{{ $foto->photo_url }}"
                           data-caption="{{ $foto->nama ?? '' }}">
                            <img src="{{ $foto->photo_url }}"
                                 alt="{{ $foto->nama ?? 'Foto Galeri' }}"
                                 loading="lazy"
        ...
        @else
            <div class="text-center py-24">
                ...
                <a href="{{ site_url('galeri') }}" class="inline-flex items-center gap-2 mt-8 text-primary font-bold hover:underline">
        ...
        @if(isset($albums) && $albums->count() > 0)
            <div class="masonry-grid">
                @foreach($albums as $idx => $album)
                    ...
                    @endphp
                    <div class="masonry-item">
                        <a href="{{ $album->detail_url }}" class="album-card block" style="height:{{ $h }}px;" title="{{ $album->nama }}">
                            @if($album->thumb_url)
                                ...
                                <div class="album-no-img" style="display:none; height:{{ $h }}px;">
                                    <i class="fas fa-images text-white/30 text-5xl"></i>
                                </div>
                            @else
                                <div class="album-no-img" style="height:{{ $h }}px;">
                                    <i class="fas fa-images text-white/30 text-5xl"></i>
                                </div>
                            @endif

                            <div class="album-overlay">
                                <div class="text-white font-bold text-sm leading-tight mb-1 truncate">{{ $album->nama }}</div>
                                <div class="flex items-center gap-2 text-white/70 text-[11px]">
                                    <i class="fas fa-images text-amber-400"></i>
                                    <span>{{ $album->children->count() }} foto</span>
...
<script>
    ...
    const lightboxPhotos = [
        @if(isset($photos) && $photos->count() > 0)
            @foreach($photos as $foto)
            { src: '{{ addslashes($foto->photo_url) }}', caption: '{{ addslashes($foto->nama ?? '') }}' },
            @endforeach
        @endif
    ];
