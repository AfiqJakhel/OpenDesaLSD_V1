        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-10">
            ...
            <div class="space-y-6">
                <a href="{{ site_url('/') }}" class="inline-flex items-center gap-4 group">
                    ...
                    <div>
                        ...
                        <p class="text-white/50 text-xs tracking-wide">{{ $desa['nama_kabupaten'] }}</p>
                    </div>
                </a>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <a href="{{ site_url('profil-desa') }}" class="text-xs text-white/60 hover:text-accent transition flex items-center gap-2">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <div class="space-y-4">
                        <!-- Email -->
                        <a href="mailto:{{ $desa['email_desa'] ?? '' }}" class="flex items-center gap-4 group">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-white/90 group-hover:text-white transition">{{ $desa['email_desa'] ?? 'kontak@desa.id' }}</p>
                            </div>
                        </a>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-white/90">{{ $desa['telepon'] ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-3">
                        @foreach ($social_media as $sm)
                            @isset($sm['link'])
                                <a href="{{ $sm['link'] }}" target="_blank" class="w-11 h-11 bg-white/5 hover:bg-accent rounded-xl flex items-center justify-center hover:text-white transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-accent/20 group">
                                    <i class="fab {{ $sm['icon'] }} text-base"></i>
                                </a>
                            @endisset
                        @endforeach
                    </div>
                            ...
                            <div class="flex items-center justify-between pb-3 border-b border-white/10">
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-lg font-bold text-white">{{ number_format($statistik_pengunjung['hari_ini'] ?? 0) }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center justify-between pb-3 border-b border-white/10">
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-lg font-bold text-white">{{ number_format($statistik_pengunjung['kemarin'] ?? 0) }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-lg font-bold text-accent">{{ number_format($statistik_pengunjung['total'] ?? 0) }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="pt-8 border-t border-white/10">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                <p class="text-xs text-white/40 text-center md:text-left">
                    &copy; {{ date('Y') }} <span class="text-white/60 font-semibold">{{ setting('sebutan_desa') }} {{ $desa['nama_desa'] }}</span>. All rights reserved.
                </p>
                <div class="flex items-center gap-6 text-xs">
                    <a href="https://opensid.my.id" target="_blank" class="text-white/40 hover:text-accent transition flex items-center gap-2">
                        ...
                        <span>OpenSID {{ ambilVersi() }}</span>
                    </a>
                    @if (setting('tte'))
                        <img src="{{ asset('assets/images/bsre.png?v', false) }}" alt="BSRE" class="h-5 opacity-40 hover:opacity-100 transition" />
                    @endif
                </div>
            </div>
        </div>
