...
class Import
{
    ...
    protected $cacheDusun = [];
    protected $cacheRw = [];
    protected $cacheRt = [];
    protected $cacheKeluarga = [];
    protected $cacheKeluargaId = [];
    protected $cacheKepalaKeluarga = [];
    protected $cachePenduduk = [];
    protected $cachePendudukNIK = [];

    protected function initCache()
    {
        $wilayah = Wilayah::select('id', 'dusun', 'rw', 'rt')->get();
        foreach($wilayah as $w) {
            $this->cacheDusun[$w->dusun] = true;
            $this->cacheRw[$w->dusun.'_'.$w->rw] = true;
            $this->cacheRt[$w->dusun.'_'.$w->rw.'_'.$w->rt] = $w->id;
        }

        $keluarga = Keluarga::select('id', 'no_kk', 'alamat', 'id_cluster')->get();
        foreach($keluarga as $k) {
            $this->cacheKeluarga[$k->no_kk] = $k;
            $this->cacheKeluargaId[$k->id] = $k;
        }

        $pk = PendudukSaja::select('id_kk')->where('kk_level', SHDKEnum::KEPALA_KELUARGA)->get();
        foreach($pk as $p) {
            $this->cacheKepalaKeluarga[$p->id_kk] = true;
        }

        $pend = PendudukSaja::select('id', 'nik', 'status_dasar', 'kk_level', 'id_kk', 'nama', 'tempatlahir', 'tanggallahir')->get();
        foreach($pend as $p) {
            $this->cachePenduduk[$p->nik] = $p;
            $this->cachePendudukNIK[$p->nama.'_'.$p->tempatlahir.'_'.$p->tanggallahir] = $p->nik;
        }
    }
     ...
     *
     * @param array $daftar_kode tulisan => kode angka
     * @param string|null $nilai tulisan yang akan dikonversi
     *
     * @return int|string|null kode angka, nilai asli kalau tidak ada kodenya
     */
    ...
    protected function tulisWilayah(&$isiBaris)
    {
        ...
        $dusunKey = $isiBaris['dusun'];
        if (!isset($this->cacheDusun[$dusunKey])) {
            $dusun = [
                [
            ...
            try { Wilayah::insert($dusun); } catch (Exception $e) {}
            $this->cacheDusun[$dusunKey] = true;
            $this->cacheRw[$dusunKey.'_0'] = true;
            $this->cacheRw[$dusunKey.'_-'] = true;
            $this->cacheRt[$dusunKey.'_0_0'] = -1;
            $this->cacheRt[$dusunKey.'_-_0'] = -1;
            $this->cacheRt[$dusunKey.'_-_-'] = -1;
        }

        // --- Masukkan rw apabila belum ada
        $rwKey = $isiBaris['dusun'] . '_' . $isiBaris['rw'];
        if (!isset($this->cacheRw[$rwKey])) {
            $rw = [
                [
                    'dusun'     => $isiBaris['dusun'],
                    'rw'        => $isiBaris['rw'],
            ...
            try { Wilayah::insert($rw); } catch (Exception $e) {}
            $this->cacheRw[$rwKey] = true;
            $this->cacheRt[$isiBaris['dusun'].'_'.$isiBaris['rw'].'_0'] = -1;
            $this->cacheRt[$isiBaris['dusun'].'_'.$isiBaris['rw'].'_-'] = -1;
        }

        // --- Masukkan rt apabila belum ada
        $rtKey = $isiBaris['dusun'] . '_' . $isiBaris['rw'] . '_' . $isiBaris['rt'];
        if (!isset($this->cacheRt[$rtKey]) || $this->cacheRt[$rtKey] === -1) {
            ...
            if (! $cekRt) {
                ...
                $rtObj                  = Wilayah::create($rt);
                $this->cacheRt[$rtKey] = $rtObj->id;
            } else {
                $this->cacheRt[$rtKey] = $cekRt->id;
            }
        }
        
        $isiBaris['id_cluster'] = $this->cacheRt[$rtKey];
    }
    ...
    protected function tulisKeluarga(&$isiBaris)
    {
        ...
        $keluargaBaru = false;
        $no_kk = $isiBaris['no_kk'];

        $data['updated_by'] = ci_auth()->id;
        $data['id_cluster'] = $isiBaris['id_cluster'];
        $data['config_id']  = identitas('id');

        if (isset($this->cacheKeluarga[$no_kk])) {
            $keluarga = $this->cacheKeluarga[$no_kk];
            ...
            if (! $keluarga->alamat) {
                // Update ke database seperlunya (hanya untuk log) 
                $kelTemp = Keluarga::find($keluarga->id);
                if ($kelTemp) {
                    $kelTemp->alamat = $isiBaris['alamat'];
                    $kelTemp->save();
                    // Sinkronisasi memori cache
                    $keluarga->alamat = $isiBaris['alamat'];
                    $this->cacheKeluarga[$no_kk] = $keluarga;
                    $this->cacheKeluargaId[$keluarga->id] = $keluarga;
                }
            }
        } else {
            ...
            try {
                ...
                // Jika terjadi gagal create (terutama Integrity constraint Duplicate entry)
                // Berarti data sudah ada di database tapi terlewat cache (mungkin soft-delete dsb)
    ...
    protected function tulisPenduduk($isiBaris)
    {
        ...
        if ($data['kk_level'] == SHDKEnum::KEPALA_KELUARGA) {
            $adaKepalaKeluarga = $this->cacheKepalaKeluarga[$isiBaris['id_kk']] ?? false;
            if ($adaKepalaKeluarga) {
                ...
                $this->cacheKepalaKeluarga[$isiBaris['id_kk']] = true;
            }
        }
        ...
        if ($isiBaris['nik'] == 0) {
            ...
            $nikKey = $isiBaris['nama'] . '_' . $isiBaris['tempatlahir'] . '_' . $isiBaris['tanggallahir'];
            if (isset($this->cachePendudukNIK[$nikKey])) {
                $isiBaris['nik'] = $this->cachePendudukNIK[$nikKey];
            } else {
                $isiBaris['nik'] = Penduduk::nikSementara();
                $this->cachePendudukNIK[$nikKey] = $isiBaris['nik'];
            }
        }
        ...
        $res = $this->cachePenduduk[$isiBaris['nik']] ?? null;
        if ($res) {
            ...
            // Abaikan no kk
            $keluarga = $this->cacheKeluargaId[$res->id_kk] ?? Keluarga::where(['id' => $res->id_kk])->first();
            ...
            // Abaikan dusun
            // Optimisasi: cari id_cluster di cache keluarga, lalu cari datanya manual via query karena Wilayah tidak terlalu banyak perubahannya
            $idCluster = $keluarga->id_cluster ?? null;
            $cluster = Wilayah::where(['id' => $idCluster])->first();
            ...
            if ($data['status_dasar'] != -1) {
                if (isset($data['tag_id_card']) && PendudukSaja::cekTagIdCard($data['tag_id_card'], $res->id)) {
            ...
            if (isset($data['tag_id_card']) && PendudukSaja::cekTagIdCard($data['tag_id_card'])) {
                return $this->errorTulisPenduduk['message'] = 'Tag ID Card ' . $data['tag_id_card'] . ' sudah digunakan pada NIK : ' . $data['nik'];
            }
            ...
            try {
                $pendObj = PendudukSaja::create($data);
                $pendudukBaru = $pendObj->id;
                
                // Masukkan ke dalam cache agar iterasi berikutnya tau data ini
                $data['id'] = $pendudukBaru;
                $this->cachePenduduk[$data['nik']] = (object) $data;
                ...
                // Tambahkan fall back apabila crash duplicate entry constraint memutus sistem
                $cekDb = PendudukSaja::where('nik', $data['nik'])->first();
                if ($cekDb) {
                    $pendudukBaru = $cekDb->id;
                    $this->cachePenduduk[$data['nik']] = $cekDb;
                } else {
    ...
    public function imporExcel($hapus = false)
    {
        try {
            ...
            foreach ($reader->getSheetIterator() as $sheet) {
                ...
                // Mengabaikan pengecekan spesifik 'Data Penduduk'
                // Sistem otomatis langsung memproses sheet pertama yang diedit
                if (! $sheetDitemukan) {
                    $sheetDitemukan = true;
                    ...
                    if (($gagal + $sukses) == 0) {
                        return set_session('error', 'Data penduduk gagal diimpor, tidak ada data valid ditemukan');
                    }

                    // Pembatasan memori sesi (Session Data Limit Cookie 4KB Preventer)
                    $pesan_array = array_filter(explode('<br>', $pesan));
                    $jumlah_pesan = count($pesan_array);
                    if ($jumlah_pesan > 30) {
                        $pesan = implode('<br>', array_slice($pesan_array, 0, 30)) . '<br><br><div class="alert alert-warning text-center"><b>... Peringatan: Terdapat ' . ($jumlah_pesan - 30) . ' baris pesan error lainnya yang disembunyikan karena terlalu panjang.</b></div>';
                    }
            ...
            if (! $sheetDitemukan) {
                return set_session('error', 'Data penduduk gagal diimpor, file Excel tidak berisi lembar (sheet) data apapun.');
            }
